    ...
    try {
        ...
        try {
            ...
            return res.status(500).json({
                message:"Ai returned response that is not vali json",
                resResponse:response.text
            })
        }
...
router.post("/resume-analyzer",async(req,res)=>{
    try {
        ...
        if(!pdfBase64){
            return res.status(400).json({
                message:"Resume pdf data is required"
            })
        }
    ...
    let  jsonResponse;
        try {
            ...
            return res.status(500).json({
                message:"Ai returned response that is not valid json",
                resResponse:response.text
            })
        }
        res.json(jsonResponse)

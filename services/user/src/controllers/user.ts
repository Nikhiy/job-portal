...
export const getUserProfile=TryCatch(async(req,res,next)=>{
    ...
    const users=await sql`
            ...
            FROM users u LEFT JOIN user_skills us ON u.user_id=us.user_id LEFT JOIN skills s ON us.skill_id=s.skill_id WHERE u.user_id=${userId}
            GROUP BY u.user_id;
            `;
...
export const updateProfilePic=TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    if(!fileBuffer || !fileBuffer.content){
        throw new ErrorHandler(500,"Failed to generate image buffer")
    }
...
export const deleteSkillFromUser=TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    `
    if(result.length===0){
        throw new ErrorHandler(404,`No skill with name ${skillName} was found`)
    }
    res.json({
